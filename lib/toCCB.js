...
const toCCB = {
    ...
    outputFile(file, fileName) {
        const outputPath = save.getCcbPath();
    ...
    createNode(pfbNode, idx) {
        ...
        // 读取设置
        if (ccbNode.displayName.substr(0, 2) == "m_") {
            ccbNode.memberVarAssignmentName = ccbNode.displayName;
            ccbNode.memberVarAssignmentType = 2;
        }
    ...
    addSprite(ccbNode, pfbCom) {
        ...
        if (pfbCom._spriteFrame) {
            ...
            if (pfbCom._atlas) {
                ...
                arr = url.split("/");
                arr = arr.splice(4, arr.length - 4 - 1);
            }
    ...
    addButton(ccbNode, pfbCom) {
        ...
        // 点击事件
        if (pfbCom['clickEvents'][0]) {
            const eventId = pfbCom['clickEvents'][0].__id__;
            const event = this.prefab[eventId];
            prop = this.getProp(properties, 'ccControl');
            prop.value[0] = event.customEventData;
            prop.value[1] = 2;
        }
