...
const toCCB = {
    ...
    startTrans(selectionUrl){
        ...
        let str = selectionUrl.split('/').pop();
        ...
        const ccbFile = this.createCCBFile(root);
        
        this.outputCCB(ccbFile);
    },

    getOutputPath() {
        const path = save.getOutput();
        const outFile = path + '/' + this.fileName + '.ccb';
        return outFile;
    },

    outputCCB(file) {
        Editor.log('outputCCB', file);
        const outFile = this.getOutputPath();
        Editor.log('outFile：', outFile);
        const ccb = plist.build(file);
        Editor.log('ccb', ccb);
        fs.writeFile(outFile, ccb, {flag: 'w'}, function (err) {
            if(err) {
                Editor.error(err);
            } else {
                Editor.log('写入成功');
    ...
    createCCBFile(root) {
        ...
        Editor.log('createCCBFile ccbFile:', ccbFile)
        const ccbLayer = this.copyTemp(CCBModule.CCBLayer);
        Editor.log('createCCBFile ccbLayer:', ccbLayer)
        ccbFile.nodeGraph = ccbLayer;
        Editor.log('createCCBFile ccbLayer.children:', ccbLayer.children)
    ...
    addSprite(ccbNode, pfbCom) {
        ...
        Editor.log('atlas:', sprite.value[0])
        Editor.log('spriteFrame:', sprite.value[1])
        Editor.log('properties:', ccbNode.properties)
